...
typedef __gnu_cxx::hash_map<StringIntTree::Node *, StringIntTree::Node *, objhash_ptr, objeq_ptr> node_node_map;

void
print_map(node_int_map &m){

  cout << "map_size = " << m.size() << endl;
  node_int_map::iterator i = m.begin();
  for ( ; i != m.end() ; ++i ){
...
NJ_MERGE_TREES(StringIntTree *t1, StringIntTree *t2, StringIntTree_DM &big_dm){
  ...
  //FNJ find current min
  node_node_map minPairMap((int)(dm.getSize()*1.3));
  for(size_t i=0;i<dm.getSize();i++){
    double minVal=FLT_MAX;
    size_t minj=10000;
    for(size_t j=0; j<dm.getSize(); j++ ){
      if(j==i) continue;
    ...
    minPairMap[dm.getIdentifier(i)] = dm.getIdentifier(minj);
  }
  //----------------
  // NJ ITERATION
  ...
  node_set usednodes;
  while ( dm.getSize() > 3 ) {
    // PRINT(dm.getSize());    print_map(rowMap);
    //find the minimal value
    double minVal = FLT_MAX;
    size_t mini = 1000000;
    size_t minj = 1000000;
    node_node_map::iterator iter = minPairMap.begin();
    for(; iter != minPairMap.end(); ++iter ){
      size_t i = rowMap[(*iter).first];
      size_t j = rowMap[(*iter).second];
      assert(i<dm.getSize());
      assert(j<dm.getSize());
      double newVal=(dm.getSize() - 2.0)*dm.getDistance(i,j) - rowSums[i] - rowSums[j];
      if(newVal<minVal){
    ...
    //PRINT(dm.getIdentifier(mini));PRINT(dm.getIdentifier(minj));
    //make sure that minj is the last row in the matrix
    if ( minj != dm.getSize() -1 ){
      
      dm.swapRowToLast(mini);
      rowMap[dm.getIdentifier(mini)] = mini;
      rowMap[dm.getIdentifier(dm.getSize()-1)] = dm.getSize()-1;
    ...
    if ( n1p == n2  ){
      ...
      newparent = n1p;
    else if ( n2p!=NULL && n2p->getParent() == n1)
      newparent = n2p;
    else{
    ...
    //---
    //FNJ check if the new node is a better sibling for the old nodes
    PENDING PROBLEM!!!
      the new parent might not be allowed
      this is also a problem in the initialization
    minPairMap.erase(n1);
    rowMap.erase(n1);
    minPairMap.erase(n2);
    rowMap.erase(n2);
    
    iter = minPairMap.begin();
    for(; iter != minPairMap.end(); ++iter ){
      if ( (*iter).second == n1 ||
           (*iter).second == n2 ){
        (*iter).second = newparent;
      }
      else{
        double currentScore = (dm.getSize() - 2.0)*dm.getDistance(rowMap[(*iter).first],rowMap[(*iter).second])
          - rowSums[rowMap[(*iter).first]] - rowSums[rowMap[(*iter).second]];
        double newparentscore =(dm.getSize() - 2.0)*dm.getDistance(rowMap[(*iter).first],mini)
          - rowSums[rowMap[(*iter).first]] - rowSums[mini];
        if ( newparentscore < currentScore )
          (*iter).second = newparent;
      }
    }

    //===--
    //FNJ find min in the new row
    minVal=FLT_MAX;
    for(size_t j=0; j<dm.getSize(); j++ ){
      if(j==mini) continue;
      double newVal=(dm.getSize() - 2.0)*dm.getDistance(mini,j) - rowSums[mini] - rowSums[j];
      if ( newVal < minVal ){
        //if the nodes are in the same tree check that they are neighbors
        StringIntTree::Node *n1 = dm.getIdentifier(mini);
        StringIntTree::Node *n2 = dm.getIdentifier(j);
    ...
    minPairMap[dm.getIdentifier(mini)] = dm.getIdentifier(minj);
    //------
  ...
  if ( n1->getTree() == n2->getTree() ){
    if ( n1->getParent() == n2 ){
      ...
      LINE();
      n1->getTree()->reRootAt(n1);
      n1->getTree()->drawTree(cout);
      n2->getTree()->reRootAt(n2);
      n2->getTree()->drawTree(cout);
      PRINT(n1->getNodeId());PRINT(n2->getNodeId());PRINT(n3->getNodeId());
      PROG_ERROR("shouldn't come here");
    }
