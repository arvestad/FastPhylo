...
void
buildTreesFloatDm(StrFloMatrix &dm, tree2int_map &tree2count, std::vector<NJ_method> &methods, str2int_hashmap &name2id){
	SequenceTree tree;
	for(size_t i=0 ; i<methods.size() ; i++){
		computeFloatNJTree(dm,tree,methods[i]);
		tree.makeCanonical(name2id);
		tree2int_map::iterator iter = tree2count.find(tree);
		if(iter!=tree2count.end())
			(*iter).second++;
		...
		if (args_info.input_format_arg == input_format_arg_binary){
			...
			while (   status == END_OF_RUN   && ( args_info.input_format_arg == input_format_arg_xml || run < args_info.number_of_runs_arg )) {
				...
				for ( i = 0 ; ( status == END_OF_RUN || status == DM_READ ) && ( i < args_info.dm_per_run_arg   || args_info.input_format_arg == input_format_arg_xml ) ; i++ ){
					...
					if (( status = istream->readFloatDM( dm, names, runId, extrainfos)) != DM_READ ) {
						break;
					};
					...
					buildTreesFloatDm(dm, tree2count, methods,name2id);
				}
