...
struct empty_b128_init
{
    void operator()(std::istream &in, DNA_b128_String &s) const
    {
    }
};
...
struct empty_b128Tree_init
{
    void operator()(std::istream &in, b128Tree::Node &s) const
    {
    }
};

typedef DistanceMatrix<b128Tree::Node *, double, empty_b128Tree_init, Data_printOn<b128Tree::Node *>, Data_init<double>,
                       Data_printOn<double>>
    b128Matrix;

//----------------------------
// FORWARD DECLARTIONS
double computeK2PDistance(DNA_b128_String &s1, DNA_b128_String &s2);
void fillMatrix(b128Matrix &dm);
...
void computeSequenceBasedNJ(std::vector<Sequence> &seqs, SequenceTree &resultTree)
{
    ...
    while (dm.getSize() > 3)
    {
        ...
        b128Tree::Node *parent = dm.getIdentifier(mini)->getTree()->detachFromParentAndAddAsSiblings(
        ...
        for (size_t i = 0; i < dm.getSize() - 1; i++)
        { // skip last row
