...
static FastdistOptions parseArgs(int argc, char **argv)
{
    ...
    app.add_option("-o,--outfile", opts.outfile, "output filename. If not specifed, output is written to stdout")
        ->type_name("filename");
    ...
    CLI::Option *seed_opt =
        app.add_option("-s,--seed", opts.seed, "Random seed. If not specified the current timestamp will be used");
    ...
    static const std::map<std::string, AmbiguityFreqModel> ambiguity_freq_map{{"UNI", AmbiguityFreqModel::Uni},
                                                                              {"BASE", AmbiguityFreqModel::Base}};
...
static void handleEarlyExitFlags(const FastdistOptions &opts)
{
    ...
    {
        cout << fastphylo_sequence_xml_relaxngstr << std::endl;
        exit(EXIT_SUCCESS);
    };
...
static void fillAndPrintAllRows(DataOutputStream &ostream, StrFloRow &dm, std::vector<DNA_b128_String> &b128seqs,
                                sequence_translation_model &trans_model, std::vector<string> &names,
                                size_t numberOfSequences, bool useFixFactor, float fixfactor, RowOutputMode mode)
{
...
static void runRowStreamingOutput(DataInputStream &istream, DataOutputStream &ostream,
                                  sequence_translation_model &trans_model, int ndatasets, int numboot,
                                  bool no_incl_orig, bool useFixFactor, float fixfactor, InputFormat input_format,
                                  RowOutputMode mode)
{
    StrFloRow dm;
    // open infile
    ...
    for (int ds = 0; ds < ndatasets || input_format == InputFormat::Xml; ds++)
    {
        ...
        { // only need to create one distance matrix
            ...
            fillAndPrintAllRows(ostream, dm, b128seqs, trans_model, names, numberOfSequences, useFixFactor, fixfactor,
                                mode);
        }
        // bootstrapping
        else
        {
            ...
            if (!istream.readSequences(seqs, runId, extrainfos))
            {
                break;
            }
            names.clear();
            ...
            ostream.printHeader(numberOfSequences);
            if (!no_incl_orig)
            { // create the distance matrix for the original sequences
                Sequences2DNA_b128(seqs, b128seqs);
                fillAndPrintAllRows(ostream, dm2, b128seqs, trans_model, names, numberOfSequences, useFixFactor,
                                    fixfactor, mode);
            }
...
static void setIdentifiersApplyFixFactorAndPrint(DataOutputStream &ostream, StrDblMatrix &dm,
                                                 std::vector<string> &names, bool useFixFactor, float fixfactor)
{
...
static void runFullMatrixOutput(DataInputStream &istream, DataOutputStream &ostream,
                                sequence_translation_model &trans_model, int ndatasets, int numboot, bool no_incl_orig,
                                bool useFixFactor, float fixfactor, InputFormat input_format)
{
    StrDblMatrix dm;
    // open infile
    //  THE DATA WE WILL PROCESS
...
int main(int argc, char **argv)
{
    ...
    try
    {
        ...
        {
            runRowStreamingOutput(*istream, *ostream, trans_model, ndatasets, numboot, no_incl_orig, useFixFactor,
                                  fixfactor, opts.input_format, RowOutputMode::Regular);
        }
        else if (opts.memory_efficient)
        {
            runRowStreamingOutput(*istream, *ostream, trans_model, ndatasets, numboot, no_incl_orig, useFixFactor,
                                  fixfactor, opts.input_format, RowOutputMode::MemoryEfficient);
        }
        else
        {
            runFullMatrixOutput(*istream, *ostream, trans_model, ndatasets, numboot, no_incl_orig, useFixFactor,
                                fixfactor, opts.input_format);
        }
    }
