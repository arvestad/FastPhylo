...
static FnjOptions parseArgs(int argc, char **argv)
{
    ...
    static const std::map<std::string, OutputFormat> output_format_map{{"newick", OutputFormat::Newick},
    ...
    static const std::map<std::string, NJ_method> method_map{{"NJ", NJ}, {"FNJ", FNJ}, {"BIONJ", BIONJ}};
...
template <class T>
void buildTrees(T &dm, tree2int_map &tree2count, std::vector<NJ_method> &methods, str2int_hashmap &name2id)
{
    SequenceTree tree;
...
static void processRuns(DataInputStream &istream, tree2int_map &tree2count, std::vector<NJ_method> &methods,
                        str2int_hashmap &name2id, std::vector<std::string> &names, std::string &runId,
                        Extrainfos &extrainfos, const FnjOptions &opts, readstatus &status)
{
    StrDblMatrix dm;
