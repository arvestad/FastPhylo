...
void XmlOutputStream::print(tree2int_map &tree2count, bool printCounts, string &runId, vector<string> &names,
                            Extrainfos &extrainfos)
{
    ...
    *fp << "    </identities>" << std::endl;
    ...
    {
        ostringstream oss;
        *fp << "    <tree>" << std::endl << "     <count>" << entry.second << "</count>" << std::endl;
        bool oldVal = xmlPrint;
        xmlPrint = true;
        *fp << "     <newick-xml>" << entry.first << "</newick-xml>" << std::endl;
        xmlPrint = false;
        *fp << "     <newick>" << entry.first << "</newick>" << std::endl << "    </tree>" << std::endl;
        xmlPrint = oldVal;
    }
    *fp << "   </run>" << std::endl;
}
