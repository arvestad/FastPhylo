...
Matrix Matrix::mult(const Matrix &lhv, const Matrix &rhv, bool tr_left, bool tr_right)
{
    ...
    dgemm_(&transl, &transr, &left_op_rows, &right_op_cols, &left_op_cols, &dummy_one,
           const_cast<double *>(&lhv.m_data[0]), &left_op_cols, const_cast<double *>(&rhv.m_data[0]), &right_op_cols,
...
MatVec Matrix::expm(const DblVec &s) const
{
    ...
    //  SUBROUTINE DGEEV( JOBVL, JOBVR, N, A, LDA, WR, WI, VL, LDVL, VR,
    //                LDVR, WORK, LWORK, INFO )
    dgeev_(&n, &v, &size, &data[0], &size, &eg_val_real[0], &eg_val_im[0], dummy, &dummy_size, &t_mat.m_data[0], &size,
           workspace_size, &w_query, info);
