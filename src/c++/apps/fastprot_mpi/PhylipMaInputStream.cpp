...
bool PhylipMaInputStream::read(std::vector<Sequence> &seqs, std::string &runId, std::vector<std::string> &names,
...
bool PhylipMaInputStream::readSequences(std::vector<Sequence> &seqs, std::string &runId, Extrainfos &extrainfos)
{
    Sequence::readSequences(seqs, *fp);
    return true;
}
