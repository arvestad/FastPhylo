...
void printPHYLIPfastSD(const StrDblMatrix &dm, FILE *out, bool writeXml, bool writeXmlSD)
{
    ...
    for (size_t i = 0; i < numNodes; i++)
    {
        ...
        for (size_t j = 0; j < entriesPerRow; j++)
        {
            ...
            if (!isfinite(f))
            {
                ...
                if (writeXml || writeXmlSD)
                {
                    //	  xmlNodePtr entryNode = xmlNewChild(rowNode,0, ( const xmlChar * ) "entry",  ( const xmlChar *
                    //) "-1" );
            ...
            if (intpart > 99)
            {
                if (f - intpart * 1.0 < 0.000001)
                {
                    if (writeXml || writeXmlSD)
                    {
                        // I guess 20 should be more than enough. Please lower this number if you know how it all works.
