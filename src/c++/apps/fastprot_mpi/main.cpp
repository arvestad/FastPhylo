    ...
    try
    {
      ...
      while (inf == 0) {
        ...
        if (rank == 0)
        {
            ...
            {
                std::cerr
                    << "The software was built with WITH_LIBXML=OFF. Please rebuild it if you want XML functionality."
                    << std::endl;
                exit(EXIT_FAILURE);
            }
#endif // WITH_LIBXML
            ...
            {
                std::cerr << "error: --print-relaxng-input and --print-relaxng-output can not be used at the same time"
                          << std::endl;
                exit(EXIT_FAILURE);
            }
            ...
            if (opts.maximum_likelihood && (trans_model.model == id || trans_model.model == jc ||
                                            trans_model.model == jck || trans_model.model == jcss))
            {
                std::cerr << "error: --maximum-likelihood can not be used with --distance-function=ID, JC, JCK or JCSS"
                          << std::endl;
                exit(EXIT_FAILURE);
            }
            ...
            try
            {
                ...
                for (int ds = 0; ds < ndatasets || opts.input_format == InputFormat::Xml; ds++)
                {
                    ...
                    if (!no_incl_orig)
                    {
                        ...
                        MPI::COMM_WORLD.Gatherv(&dv[0], local_nr_dists, MPI::DOUBLE, &dv[0], &rec_count[0], &displ[0],
                                                MPI::DOUBLE, 0);
        ...
        { // Worker
            ...
            MPI::COMM_WORLD.Gatherv(&dv[0], local_nr_dists, MPI::DOUBLE, &dv[0], 0, 0, MPI::DOUBLE, 0);
        }
