 ...
 * @param tm Translation model that contains parameters for the calculations
 */
void calculate_ed_dists_mpi(const SeqVec &sv, std::vector<double> &dv, const prot_sequence_translation_model &tm,
                            int rank, int size)
{
...
void calculate_stormsonnhammer_dists(const SeqVec &sv, StrDblMatrix &dm)
{
    for (int i = 0; i < sv.size(); i++)
    {
        for (int j = i + 1; j < sv.size(); j++)
        {
            ...
            {
                adj_distance = -100 * log(1 - 0.95844 * diff - 0.69957 * pow(diff, 2) + 2.4955 * pow(diff, 3) -
