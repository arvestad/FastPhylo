...
XmlInputStream::XmlInputStream(char *filename)
{
    ...
    xmlRelaxNGSetParserErrors(parserctxt, (xmlRelaxNGValidityErrorFunc)fprintf, (xmlRelaxNGValidityWarningFunc)fprintf,
                              stderr);
...
bool XmlInputStream::read(std::vector<Sequence> &seqs, std::string &runId, std::vector<std::string> &names,
...
bool XmlInputStream::readSequences(std::vector<Sequence> &seqs, std::string &runId, Extrainfos &extrainfos)
{
    ...
    while ((ret = xmlTextReaderRead(reader)) == 1)
    {
        ...
        if (l.in_root && l.in_runs && l.in_run && l.in_seq && depth == 4 &&
            xmlStrEqual(name, (const xmlChar *)"extrainfo"))
        {
            if (type == XML_READER_TYPE_ELEMENT)
            {
