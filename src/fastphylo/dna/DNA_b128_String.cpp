...
//-----------------------------------------
//  MEMORY ALLOCATION
void DNA_b128_String::_init_mem(int capacity)
{
...
void DNA_b128_String::_free_mem()
{
    if (data != nullptr)
    {
        free_b128(data);
...
//-----------------------------------------
// APPEND
int DNA_b128_String::append(const char *c_str)
{
    ...
    for (; true; c_str++)
    {
        ...
        case 'M': {
            _fastSetNucleotideUnknown(c_pos);
            ambiguities.push_back(ambiguity_nucleotide_at_position{{DNA_M_, 0.5, 0.5, 0, 0}, c_pos});
            c_int = c_int | DNA_A_;
        }
        break; // 	A or C          K
        case 'r':
        case 'R': {
            _fastSetNucleotideUnknown(c_pos);
            ambiguities.push_back(ambiguity_nucleotide_at_position{{DNA_R_, 0.5, 0, 0.5, 0}, c_pos});
            c_int = c_int | DNA_A_;
        }
        break; // 	A or G 	        Y
        case 'w':
        case 'W': {
            _fastSetNucleotideUnknown(c_pos);
            ambiguities.push_back(ambiguity_nucleotide_at_position{{DNA_W_, 0.5, 0, 0, 0.5}, c_pos});
            c_int = c_int | DNA_A_;
        }
        break; // 	A or T 	        W
        case 's':
        case 'S': {
            _fastSetNucleotideUnknown(c_pos);
            ambiguities.push_back(ambiguity_nucleotide_at_position{{DNA_S_, 0, 0.5, 0.5, 0}, c_pos});
            c_int = c_int | DNA_A_;
        }
        break; // 	C or G 	        S
        case 'y':
        case 'Y': {
            _fastSetNucleotideUnknown(c_pos);
            ambiguities.push_back(ambiguity_nucleotide_at_position{{DNA_Y_, 0, 0.5, 0, 0.5}, c_pos});
            c_int = c_int | DNA_A_;
        }
        break; // 	C or T 	        R
        case 'k':
        case 'K': {
            _fastSetNucleotideUnknown(c_pos);
            ambiguities.push_back(ambiguity_nucleotide_at_position{{DNA_K_, 0, 0, 0.5, 0.5}, c_pos});
            c_int = c_int | DNA_A_;
        }
        break; // 	G or T 	        M
        case 'v':
        case 'V': {
            _fastSetNucleotideUnknown(c_pos);
            ambiguities.push_back(ambiguity_nucleotide_at_position{{DNA_V_, 0.333333, 0.333333, 0.333333, 0}, c_pos});
            c_int = c_int | DNA_A_;
        }
        break; // 	A or C or G 	B
        case 'h':
        case 'H': {
            _fastSetNucleotideUnknown(c_pos);
            ambiguities.push_back(ambiguity_nucleotide_at_position{{DNA_H_, 0.333333, 0.333333, 0, 0.333333}, c_pos});
            c_int = c_int | DNA_A_;
        }
        break; // 	A or C or T 	D
        case 'd':
        case 'D': {
            _fastSetNucleotideUnknown(c_pos);
            ambiguities.push_back(ambiguity_nucleotide_at_position{{DNA_D_, 0.333333, 0, 0.333333, 0.333333}, c_pos});
            c_int = c_int | DNA_A_;
        }
        break; // 	A or G or T 	H
        case 'b':
        case 'B': {
            _fastSetNucleotideUnknown(c_pos);
            ambiguities.push_back(ambiguity_nucleotide_at_position{{DNA_B_, 0, 0.333333, 0.333333, 0.333333}, c_pos});
            c_int = c_int | DNA_A_;
        }
        break; // 	C or G or T 	V
        case 'n':
        ...
        case '?': {
            _fastSetNucleotideUnknown(c_pos);
            ambiguities.push_back(ambiguity_nucleotide_at_position{{DNA_N_, 0.25, 0.25, 0.25, 0.25}, c_pos});
            c_int = c_int | DNA_A_;
        }
...
nucleotide DNA_b128_String::getNucleotide(int pos) const
{
...
nucleotide DNA_b128_String::getNucleotideNOAMBIGUITY(int pos) const
{
...
nucleotide DNA_b128_String::setNucleotide(int pos, nucleotide n)
{
    ...
    default:
        ...
        auto iter = ambiguities.begin();
        while (iter != ambiguities.end() && (*iter).position < pos)
        {
            ++iter;
        }
...
std::ostream &operator<<(std::ostream &out, const DNA_b128_String &s)
{
    ...
    for (int i = 0; i < s.getNumChars(); i++)
    {
        out << nucleotide2char(s.getNucleotide(i));
    }
...
// RESOLVE AMBIGUITIES
void DNA_b128_String::resolveAmbiguities(const DNA_b128_String &temp_str)
{
...
void DNA_b128_String::resolveAmbiguitiesUsingTransitionProbabilities(const DNA_b128_String &temp_str,
    ...
    for (; iter != ambiguities.end(); ++iter)
    {
        ...
        if (template_nucleotide != DNA_UNKNOWN_)
        {
            ...
            if (is_ambiguity_contained(tmp_amb, ambpos.ambiguity))
            {
                ...
                {
                case DNA_A_:
                    totalprob = (curramb.probA * mldist.A_A) + (curramb.probC * mldist.A_C) +
                                (curramb.probG * mldist.A_G) + (curramb.probT * mldist.A_T);
                    ...
                    break;
                case DNA_C_:
                    totalprob = (curramb.probA * mldist.A_C) + (curramb.probC * mldist.C_C) +
                                (curramb.probG * mldist.C_G) + (curramb.probT * mldist.C_T);
                    ...
                    break;
                case DNA_G_:
                    totalprob = (curramb.probA * mldist.A_G) + (curramb.probC * mldist.C_G) +
                                (curramb.probG * mldist.G_G) + (curramb.probT * mldist.G_T);
                    ...
                    break;
                case DNA_T_:
                    totalprob = (curramb.probA * mldist.A_T) + (curramb.probC * mldist.C_T) +
                                (curramb.probG * mldist.G_T) + (curramb.probT * mldist.T_T);
...
Distance DNA_b128_String::correctDistanceWithAmbiguitiesUsingBackgroundFrequencesImpl(Distance sp,
...
template <typename Distance, typename Accumulate>
// NOLINTNEXTLINE(readability-function-cognitive-complexity)
Distance DNA_b128_String::correctDistanceWithAmbiguitiesUsingTransitionProbabilitiesImpl(
    Distance sp, ML_string_distance tp, const DNA_b128_String &s1, const DNA_b128_String &s2, Accumulate accumulate)
{
    Distance real_distance = sp;
...
simple_string_distance DNA_b128_String::correctDistanceWithAmbiguitiesUsingBackgroundFrequences(
    simple_string_distance sp, const DNA_b128_String &s1, const DNA_b128_String &s2)
{
    return correctDistanceWithAmbiguitiesUsingBackgroundFrequencesImpl(
        sp, s1, s2, [](simple_string_distance &d, ambiguity_distance amdist) {
...
TN_string_distance DNA_b128_String::correctDistanceWithAmbiguitiesUsingBackgroundFrequences(TN_string_distance sp,
                                                                                            const DNA_b128_String &s1,
                                                                                            const DNA_b128_String &s2)
{
    return correctDistanceWithAmbiguitiesUsingBackgroundFrequencesImpl(
        sp, s1, s2, [](TN_string_distance &d, ambiguity_distance amdist) {
...
simple_string_distance DNA_b128_String::correctDistanceWithAmbiguitiesUsingTransitionProbabilities(
    simple_string_distance sp, ML_string_distance tp, const DNA_b128_String &s1, const DNA_b128_String &s2)
{
    return correctDistanceWithAmbiguitiesUsingTransitionProbabilitiesImpl(
        sp, tp, s1, s2, [](simple_string_distance &d, ambiguity_distance amdist) {
            d.transitions += amdist.purine_transition_prob + amdist.pyrimidine_transition_prob;
            d.transversions += amdist.transversion_prob;
            d.deletedPositions -= 1;
        });
...
TN_string_distance DNA_b128_String::correctDistanceWithAmbiguitiesUsingTransitionProbabilities(
    TN_string_distance sp, ML_string_distance tp, const DNA_b128_String &s1, const DNA_b128_String &s2)
{
    return correctDistanceWithAmbiguitiesUsingTransitionProbabilitiesImpl(
        sp, tp, s1, s2, [](TN_string_distance &d, ambiguity_distance amdist) {
            d.purine_transitions += amdist.purine_transition_prob;
            d.pyrimidine_transitions += amdist.pyrimidine_transition_prob;
            d.transversions += amdist.transversion_prob;
            d.deletedPositions -= 1;
