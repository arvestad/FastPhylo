...
{
    return (d1.deletedPositions == d2.deletedPositions) && (d1.transitions == d2.transitions) &&
           (d1.transversions == d2.transversions);
}
bool operator==(const TN_string_distance &d1, const TN_string_distance &d2)
{
    return (d1.deletedPositions == d2.deletedPositions) && (d1.purine_transitions == d2.purine_transitions) &&
           (d1.pyrimidine_transitions == d2.pyrimidine_transitions) && (d1.transversions == d2.transversions);
}

std::ostream &operator<<(std::ostream &os, const simple_string_distance &d)
{
    os << "{ deletedPositions = " << d.deletedPositions << " , transitions = " << d.transitions
       << " , transversions = " << d.transversions << " }";
    return os;
}

std::ostream &operator<<(std::ostream &os, const TN_string_distance &d)
{
    os << "{ deletedPositions = " << d.deletedPositions << " , purine_trans = " << d.purine_transitions
       << " , pyrimidine_trans = " << d.pyrimidine_transitions << " , transversions = " << d.transversions << " }";
    return os;
}
