...
static void inline PREFETCH_DATA(size_t numDatas)
{
    b128 *_del_ptr1;
    b128 *_del_ptr2;
    b128 *_ptr1;
...
static inline void sum_with_previous_level(b128 &sum_current, b128 sum_previous, b128 mask, b128 shift)
{
    sum_current = add_b128(sum_current, add_b128(and_b128(sum_previous, mask),
                                                 and_b128(shift_each32_bits_right_b128(sum_previous, shift), mask)));
}
...
#define SUM_WITH_PREVIOUS_LEVEL_IMMEDIATEBYTESHIFT(SUM_CURRENT, SUM_PREVIOUS, MASK, BYTESHIFT)                         \
    {                                                                                                                  \
        (SUM_CURRENT) =                                                                                                \
            add_b128((SUM_CURRENT), add_b128(and_b128((SUM_PREVIOUS), (MASK)),                                         \
                                             and_b128(shift_bytes_right_b128<(BYTESHIFT)>((SUM_PREVIOUS)), (MASK))));  \
    }

// CONVERTS THE SUM
// Takes the sum of the previous level adds all two adjacent blocks into
// one block of the next level.
static inline void convert_sum(b128 &sum_current, b128 sum_previous, b128 mask, b128 shift)
{
    sum_current =
        add_b128(and_b128(sum_previous, mask), and_b128(shift_each32_bits_right_b128(sum_previous, shift), mask));
}
...
#define CONVERT_SUM_IMMEDIATEBYTESHIFT(SUM_CURRENT, SUM_PREVIOUS, MASK, BYTESHIFT)                                     \
    {                                                                                                                  \
        (SUM_CURRENT) = add_b128(and_b128((SUM_PREVIOUS), (MASK)),                                                     \
                                 and_b128(shift_bytes_right_b128<(BYTESHIFT)>((SUM_PREVIOUS)), (MASK)));               \
    }
...
static void sumRemainderBlocks(int rest_num_b128s, b128 &total_sum_ts, b128 &total_sum_pyrts, b128 &total_sum_tv,
                               b128 &total_sum_del)
{
    b128 diff;
    b128 del;
    ...
    case 2:
        ...
        total_sum_pyrts =
            and_b128(shift_each32_bits_right_b128(and_b128(get_b128(ptr1), get_b128(ptr2)), ONE), total_sum_ts);
    ...
    case 1: {
        ...
        total_sum_pyrts =
            add_b128(total_sum_pyrts,
                     and_b128(shift_each32_bits_right_b128(and_b128(get_b128(ptr1), get_b128(ptr2)), ONE), tmp_ts));
...
TN_string_distance DNA_b128_String::computeTAMURANEIDistance(const DNA_b128_String &s1, const DNA_b128_String &s2)
{
...
//
void dist_level_2(b128 &sum_ts_l2, b128 &sum_pyrts_l2, b128 &sum_tv_l2, b128 &sum_del_l2)
{
    b128 sum_ts_l1;
    b128 sum_pyrts_l1;
    b128 sum_tv_l1;
...
// level 3.
void dist_level_3(b128 &sum_ts_l3, b128 &sum_pyrts_l3, b128 &sum_tv_l3, b128 &sum_del_l3)
{
    b128 sum_ts_l2;
    b128 sum_pyrts_l2;
    b128 sum_tv_l2;
...
//
//
void dist_level_4(b128 &sum_ts_l4, b128 &sum_pyrts_l4, b128 &sum_tv_l4, b128 &sum_del_l4)
{
    b128 sum_ts_l3;
    b128 sum_pyrts_l3;
    b128 sum_tv_l3;
