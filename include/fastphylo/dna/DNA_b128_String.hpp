...
class DNA_b128_String : public Object
{
  public:
    ...
    static simple_string_distance computeDistance(const DNA_b128_String &s1, const DNA_b128_String &s2);
    static TN_string_distance computeTAMURANEIDistance(const DNA_b128_String &s1, const DNA_b128_String &s2);

    // the number of gaps is the number of missmatches of '-' toward something that is not '-'.
    static int getNumberOfGaps(const DNA_b128_String &s1, const DNA_b128_String &s2, simple_string_distance d)
    {
    ...
    static Distance correctDistanceWithAmbiguitiesUsingBackgroundFrequencesImpl(Distance sp, const DNA_b128_String &s1,
                                                                                const DNA_b128_String &s2,
                                                                                Accumulate accumulate);
    template <typename Distance, typename Accumulate>
    static Distance correctDistanceWithAmbiguitiesUsingTransitionProbabilitiesImpl(Distance sp, ML_string_distance tp,
                                                                                   const DNA_b128_String &s1,
                                                                                   const DNA_b128_String &s2,
                                                                                   Accumulate accumulate);
    ...
    void calcAmbiguityProbabilities(size_t basefreqA, size_t basefreqC, size_t basefreqG, size_t basefreqT)
    {
        std::vector<ambiguity_nucleotide_at_position>::iterator iter = ambiguities.begin();
        for (; iter != ambiguities.end(); ++iter)
        {
            (*iter).ambiguity =
                nucleotide2ambiguity_nucleotide((*iter).ambiguity.n, basefreqA, basefreqC, basefreqG, basefreqT);
        }
    }
  ...
  private:
    ...
    //
    void _fastSetNucleotideUnknown(int data_pos, int int_in_data_pos, int char_in_int_pos)
    {
    ...
    void _fastClearNucleotideUnknown(int data_pos, int int_in_data_pos, int char_in_int_pos)
    {
        b128 c_b128 = get_b128(unknownData + data_pos);
        int c_int = get_int_b128(c_b128, int_in_data_pos);
    ...
    {
        std::cout << "charpos: " << charpos << "    D_I: " << D_I(charpos) << "  INT_D_I: " << INT_D_I(charpos)
                  << "   BIT_INT_I: " << BIT_INT_I(charpos) << std::endl;
    }
};
