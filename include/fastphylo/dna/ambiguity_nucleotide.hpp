...
static __inline bool operator==(const ambiguity_nucleotide &a1, const ambiguity_nucleotide &a2)
{

    return (a1.n == a2.n && a1.probA == a2.probA && a1.probC == a2.probC && a1.probG == a2.probG &&
            a1.probT == a2.probT);
}
...
// functions.
static __inline ambiguity_nucleotide nucleotide2ambiguity_nucleotideUNIFORM(const nucleotide &n)
{

    switch (n)
    { //                                  A     C    G    T
    case DNA_A_: {
        return ambiguity_nucleotide{n, 1.0, 0, 0, 0};
    }
    case DNA_C_: {
        return ambiguity_nucleotide{n, 0, 1.0, 0, 0};
    }
    case DNA_G_: {
        return ambiguity_nucleotide{n, 0, 0, 1.0, 0};
    }
    case DNA_T_: {
        return ambiguity_nucleotide{n, 0, 0, 0, 1.0};
    }
    case DNA_M_: {
        return ambiguity_nucleotide{n, 0.5, 0.5, 0, 0};
    }
    case DNA_R_: {
        return ambiguity_nucleotide{n, 0.5, 0, 0.5, 0};
    } // an.probA = 0.5; an.probG = 0.5; break;
    case DNA_W_: {
        return ambiguity_nucleotide{n, 0.5, 0, 0, 0.5};
    } // an.probA = 0.5; an.probT = 0.5; break;
    case DNA_S_: {
        return ambiguity_nucleotide{n, 0, 0.5, 0.5, 0};
    } // an.probC = 0.5; an.probG = 0.5; break;
    case DNA_Y_: {
        return ambiguity_nucleotide{n, 0, 0.5, 0, 0.5};
    } // an.probC = 0.5; an.probT = 0.5; break;
    case DNA_K_: {
        return ambiguity_nucleotide{n, 0, 0, 0.5, 0.5};
    } // an.probG = 0.5; an.probT = 0.5; break;
    case DNA_V_: {
        return ambiguity_nucleotide{n, 0.333333, 0.333333, 0.333333, 0};
    } // an.probA = 0.999999/3.0; an.probC = 0.999999/3.0; an.probG = 0.999999/3.0; break;
    case DNA_H_: {
        return ambiguity_nucleotide{n, 0.333333, 0.333333, 0, 0.333333};
    } // an.probA = 0.999999/3.0; an.probC = 0.999999/3.0; an.probT = 0.999999/3.0; break;
    case DNA_D_: {
        return ambiguity_nucleotide{n, 0.333333, 0, 0.333333, 0.333333};
    } // an.probA = 0.999999/3.0; an.probG = 0.999999/3.0; an.probT = 0.999999/3.0; break;
    case DNA_B_: {
        return ambiguity_nucleotide{n, 0, 0.333333, 0.333333, 0.333333};
    } // an.probC = 0.999999/3.0; an.probG = 0.999999/3.0; an.probT = 0.999999/3.0; break;
    case DNA_N_: {
...
static __inline ambiguity_nucleotide regularnucleotide2ambiguity_nucleotide(const nucleotide &n)
{
...
static __inline ambiguity_nucleotide nucleotide2ambiguity_nucleotide(const nucleotide &n, size_t basefreqA = 1,
                                                                     size_t basefreqC = 1, size_t basefreqG = 1,
                                                                     size_t basefreqT = 1)
{
    ...
    case DNA_M_:
        ...
        an.probC = (0.999999 * C) / (A + C);
        break;
    case DNA_R_:
        an.probA = (0.999999 * A) / (A + G);
        an.probG = (0.999999 * G) / (A + G);
        break;
    case DNA_W_:
        an.probA = (0.999999 * A) / (A + T);
        an.probT = (0.999999 * T) / (A + T);
        break;
    case DNA_S_:
        an.probC = (0.999999 * C) / (C + G);
        an.probG = (0.999999 * G) / (C + G);
        break;
    case DNA_Y_:
        an.probC = (0.999999 * C) / (C + T);
        an.probT = (0.999999 * T) / (C + T);
        break;
    case DNA_K_:
        an.probG = (0.999999 * G) / (G + T);
        an.probT = (0.999999 * T) / (G + T);
        break;
    case DNA_V_:
...
// two nucleotides.
static __inline ambiguity_distance compute_ambiguity_distance(const ambiguity_nucleotide &a1,
                                                              const ambiguity_nucleotide &a2)
{
    ...
    //   assert ( 0.999995 <= matchprob + dist.purine_transition_prob +dist.pyrimidine_transition_prob +
    //   dist.transversion_prob ); assert ( 1.0 >= matchprob + dist.purine_transition_prob
    //   +dist.pyrimidine_transition_prob  + dist.transversion_prob);

    return dist;
}

// using transition probabilities
static __inline ambiguity_distance compute_ambiguity_distance_using_transition_probabilities(
    const ambiguity_nucleotide &a1, const ambiguity_nucleotide &a2, const ML_string_distance &tp)
{
    ...
    //-----------------------------------
    float totalprob =
        matchprob + dist.purine_transition_prob + dist.pyrimidine_transition_prob + dist.transversion_prob;
    ...
    if (totalprob == 0)
    {
        ...
        dist.transversion_prob = a1.probA * a2.probC + a1.probA * a2.probT + a1.probC * a2.probA + a1.probT * a2.probA +
                                 a1.probG * a2.probC + a1.probG * a2.probT + a1.probC * a2.probG + a1.probT * a2.probG;
        matchprob = 0;
    ...
    // std::cout << "sum " << matchprob + dist.purine_transition_prob +dist.pyrimidine_transition_prob +
    // dist.transversion_prob << std::endl; things that need to be true
    assert(matchprob <= 1.0);
    assert(matchprob >= 0.0);
    assert(dist.purine_transition_prob <= 1.0);
    assert(dist.purine_transition_prob >= 0.0);
...
//
static __inline ambiguity_distance compute_ambiguity_distance_using_transition_probabilities(
    const nucleotide &n, const ambiguity_nucleotide &a2, const ML_string_distance &tp)
{
    // SPEED UP
    float matchprob;
...
static __inline bool is_ambiguity_contained(const ambiguity_nucleotide &n, const ambiguity_nucleotide &set)
{
