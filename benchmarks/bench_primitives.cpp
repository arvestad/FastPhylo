...
int main()
{ // NOLINT(bugprone-exception-escape) - a benchmark binary crashing on an unexpected exception is a fine, loud failure
  // mode.
    const int WARMUP = 3;
    const int REPS = 21;
    ...
    for (unsigned long len : lengths)
    {
        ...
        Stats new_id = time_calls([&]() { sink_d += count_id_fraction(c1.data(), c1.size(), c2.data(), c2.size()); },
                                  WARMUP, REPS);
        std::cout << "count_id_dist," << len << "," << old_id.median_us << "," << old_id.q1_us << "," << old_id.q3_us
        ...
        Stats new_rep = time_calls(
            ...
            WARMUP, REPS);
        std::cout << "count_replacements," << len << "," << old_rep.median_us << "," << old_rep.q1_us << ","
                  << old_rep.q3_us << "," << new_rep.median_us << "," << new_rep.q1_us << "," << new_rep.q3_us << ","
                  << (old_rep.median_us / new_rep.median_us) << "\n";

        if (sink_d == 123456.0 && sink_s == 123456)
        {
